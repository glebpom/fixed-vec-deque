    ...
    T::Item: Default,
{
    ...
    pub fn new_from_array(mut data: T) -> Self {
        let m = data.ptr_mut();

        for o in 0..T::size() {
            unsafe { ptr::write(m.add(o), T::Item::default()); }
        }
...
macro_rules! impl_array(
    ($($size:expr),+) => {
        $(
            unsafe impl<T> $crate::Array for [T; $size] where T: Default {
                type Item = T;
                fn size() -> usize { $size }
